...
async def test_get_client_by_phone_number(monkeypatch):
    ...
    result = await service.get_client_by_phone_number(db_mock := AsyncMock(), phone_number)
    ...
    monkeypatch.setattr(schema.Client, "from_orm", from_orm_mock := MagicMock(return_value=expected_result))

    result = await service.get_client_by_phone_number(db_mock, phone_number)
...
async def test_update_client_none(monkeypatch):
    ...
    result = await service.update_client(db_mock := AsyncMock(), client_mock := MagicMock())
    assert result == expected_result
...
async def test_delete_client_none(monkeypatch):
    ...
    result = await service.delete_client(db_mock := AsyncMock(), client_id)
    assert result == expected_result
...
async def test_get_clients_by_tag(monkeypatch):
    ...
    monkeypatch.setattr(schema.Client, "from_orm", from_orm_mock := MagicMock(return_value="Result"))

    result = await service.get_clients_by_tag(db_mock := AsyncMock(), tag_mock := MagicMock())
...
async def test_get_clients_by_tags(monkeypatch):
    ...
    monkeypatch.setattr(schema.Client, "from_orm", from_orm_mock)

    result = await service.get_clients_by_tags(db_mock := AsyncMock(), tags_mock := [MagicMock()])
...
async def test_get_clients_by_phone_code(monkeypatch):
    ...
    result = await service.get_clients_by_phone_code(db_mock := AsyncMock(), tag_mock := MagicMock())
...
async def test_get_clients_by_phone_codes(monkeypatch):
    ...
    result = await service.get_clients_by_phone_codes(db_mock := AsyncMock(), tags_mock := [MagicMock()])
